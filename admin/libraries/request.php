 ...
 * @date	16:11:02 2012-12-17
 * @version	$Id: Request.php 11 2012-12-19 03:24:50Z gangzong $
...
//		}
	...
	public function curl_init()
	{
		...
		$ch = RequestPool::get_curl($this->get_host_id());
	...
	public function send()
	{
		...
		RequestPool::reset_curl_state($this->get_host_id(), $this->get_curl_id());
		$this->reset_ch();
	...
	{
		if (is_array($input)) {
			return array_map(array('Unit_Http_Request', 'urlencode'), $input);
	...
	{
		if (is_array($input)) {
			return array_map(array('Unit_Http_Request', 'urlencode_raw'), $input);
		} else if (is_scalar($input)) {
	...
	{
		if (is_array($input)) {
			return array_map(array('Unit_Http_Request', 'urlencode_rfc3986'), $input);
		} else if (is_scalar($input)) {
